...
class UserController {
    static async signup(req: Request, res: Response, next: NextFunction) { 
        try {
            ...
            res.status(201).json({ ok: true, message: 'Signed up successfully. Please verify your account before logging in.' });
    ...
    static async login(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const token = JWT.generateJwt(String(user._id), user.email, user.username, role);
